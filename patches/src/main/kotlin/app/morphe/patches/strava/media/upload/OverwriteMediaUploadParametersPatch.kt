// Forked from: https://github.com/ReVanced/revanced-patches/blob/6b06b9d1328b971a06d10b4247f4c10f050e4f61/patches/src/main/kotlin/app/revanced/patches/strava/media/upload/OverwriteMediaUploadParametersPatch.kt
...
val overwriteMediaUploadParametersPatch = bytecodePatch(
    ...
    execute {
        compressionQuality?.let { compressionQuality ->
            getCompressionQualityFingerprint.method.returnEarly(compressionQuality / 100f)
        }

        maxDuration?.let { maxDuration ->
            getMaxDurationFingerprint.method.returnEarly(maxDuration)
        }

        maxSize?.let {
            getMaxSizeFingerprint.method.returnEarly(it)
        }
    }
}
