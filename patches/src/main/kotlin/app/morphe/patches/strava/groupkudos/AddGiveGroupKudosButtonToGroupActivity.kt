// Forked from: https://github.com/ReVanced/revanced-patches/blob/6b06b9d1328b971a06d10b4247f4c10f050e4f61/patches/src/main/kotlin/app/revanced/patches/strava/groupkudos/AddGiveGroupKudosButtonToGroupActivity.kt
...
val addGiveGroupKudosButtonToGroupActivity = bytecodePatch(
    ...
    execute {
        ...
        // Morphe 1.1+ makes PatchClasses class insertion internal, so we invoke it via reflection to maintain compatibility with the original ReVanced patch
        val patchClassesMethod = this.javaClass.methods.find { it.name.startsWith("getPatchClasses") }
            ?: throw IllegalStateException("getPatchClasses not found in context")
        val patchClasses = patchClassesMethod.invoke(this)
        val addClassMethod = patchClasses.javaClass.methods.find { it.name.startsWith("addClass") }
            ?: throw IllegalStateException("addClass not found in patchClasses")
